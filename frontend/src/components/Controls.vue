<template>
    <div id="actions" v-if="isLogged">
        <h2 class="pull-left">Hola {{userName}} <a :href="endpoint+service+`logout`">Logout</a></h2>
        <span v-if="!authError">
...
<script>
  ...
  export default {
    ...
    props : ['isRoot','isRootForUser', 'userName', 'isLogged', 'authError', 'readWrite', 'endpoint','service'],
    methods: {
        data() {
            return {
